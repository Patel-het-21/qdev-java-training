 ...
 * <p>
 * This controller exposes endpoints for user registration, login, logout, and
 * validation checks for existing company names and email addresses.
 * </p>
 */
@RestController
@RequestMapping("/api/v1/auth")
@Validated
@CrossOrigin(origins = { "http://localhost:4200", "http://localhost:9090" }, allowCredentials = "true")
public class AuthController {
	 ...
	 * <p>
	 * This endpoint creates a new user in the master database and initializes the
	 * tenant database/schema for the company.
	 * </p>
	 *
	...
	public ResponseEntity<UserResponseDto> register(@Valid @RequestBody RegisterRequestDto dto) {
		...
		return ResponseEntity.ok(new UserResponseDto(user.getId(), user.getCompanyName(), user.getEmail(), tenant.getSchemaName()));
	}

	/**
	 * Authenticates a user using email and password.
	 * <p>
	 * On successful authentication, the tenant schema is resolved and returned
	 ...
	 * @return {@code true} if the company name already exists, otherwise
	 *         {@code false}
	 */
