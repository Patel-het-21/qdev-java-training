...
export class EmployeeListComponent implements OnInit {
  ...
  search() {
    ...
    this.filteredEmployees = this.employees.filter(emp => {
      ...
      const dob = emp.dateOfBirth ? new Date(emp.dateOfBirth).toLocaleDateString('en-GB').replace(/\//g, '-') : '';
      const age = emp.age?.toString() || '';
      const email = (emp.email || '').toLowerCase();
      const address = ((emp.address1 || '') + ' ' + (emp.address2 || '')).toLowerCase();
